...
PR #11で、保存済みTrajectoryから決定論的に`EpisodeEvaluation`を作る評価境界を固定した。
この段階ではEvaluationは「何が起きたか」を表し、scalar rewardやpolicy updateを含まない。

Reward Contract v0は、そのEvaluationへ明示的な価値判断を適用して、説明可能な学習信号へ写像する最小境界を追加する。
